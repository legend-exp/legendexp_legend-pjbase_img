...
* Julia v1.12 (default), v1.11 and v1.10
* PyTorch and JAX
...
Pull the image via
    ...
    shifterimg pull docker:legendexp/legend-pjbase:latest

Then try running an interactive session via

    shifter --image docker:legendexp/legend-pjbase:latest -- /bin/bash
...
With Singularity v2.x, convert the Docker image to a Singularity image via
    ...
    sudo singularity build legend-pjbase.sqsh docker://legendexp/legend-pjbase:latest

With Singularity v3.x, you'll probably want to use the new SIF container format:

    sudo singularity build legend-pjbase.sif docker://legendexp/legend-pjbase:latest
...
Afterwards, try running an interactive session via
    ...
    singularity shell /path/to/legend-pjbase.[sqsh|sif]

On systems with NVIDIA GPUs, use

    singularity shell --nv /path/to/legend-pjbase.[sqsh|sif]
...
```shell
docker pull legendexp/legend-pjbase:latest
docker run -it --name mylegendinstance legendexp/legend-pjbase:latest
```
...
* Bind directory `"/root"` inside the container to directory `"legend-pjbase"` in your home on the host machine (option `-v "$HOME/legend-pjbase":"/root":delegated`). Since `"$HOME"` is set to `"/root"` inside the container, this ensure that all files created in `"$HOME"` by the container (e.g. config files like Jupyter configuration, Julia packages, etc.) end up in `"legend-pjbase"` on the host system, instead of messing up your home directory. We'll use the `delegated` [mount semantics](https://docs.docker.com/docker-for-mac/osxfs-caching/) to reduce latency.
...
We'll start a container running a Bash shell, which will have access to all software the container provides. Within the Docker container, you will be user "root". "$HOME" will point to `"/root"`, which is kept in sync, bi-directionally and in (almost) real time, to the directory "$HOME/legend-pjbase" outside of the container (see above). Likewise, `"/home/user"` within the container will be kept in sync with `"$HOME"` outside of the container.
...
docker run -it --rm \
    -v "$HOME":"/home/user" \
    -v "$HOME/legend-pjbase":"/root":delegated \
    -p 8888:8888 \
    -p 14500:14500 \
    legendexp/legend-pjbase:latest
```

To start the container and run a different program than `bash` inside of it directly (not via the shell), use

```shell
docker run [...options...] legendexp/legend-pjbase:latest PROGRAM_TO_RUN ARGS...
```
